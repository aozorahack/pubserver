(function() {
  var AdmZip, DATA_LIFETIME, DEFAULT_LIMIT, GridStore, MongoClient, add_ogp, api_root, app, bodyParser, check_archive, compression, content_type, express, fs, get_from_cache, get_ogpcard, get_zipped, methodOverride, mongo_url, mongodb, mongodb_credential, mongodb_host, mongodb_port, morgan, re_or_str, redis, repo_backend, request, upload_content, upload_content_data, version, yaml, zlib;
  ...
  get_from_cache = function(my, book_id, get_file, ext, cb) {
    ...
    return my.rc.get(key, function(err, result) {
      if (err || !result) {
        if (get_file) {
          return get_file(my, book_id, function(err, data) {
            if (err) {
              return cb(err);
            } else {
