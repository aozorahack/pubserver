(function() {
  ...
  app.route(api_root + '/books').get(function(req, res) {
    ...
    if (req.query.title) {
      query['title'] = {
        "$in": [new RegExp(req.query.title)]
      };
    }
    if (req.query.author) {
      query['authors.full_name'] = {
        "$in": [new RegExp(req.query.author)]
      };
    }
    if (req.query.after) {
      query['release_date'] = {
