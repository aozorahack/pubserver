(function() {
  var AdmZip, DATA_LIFETIME, DEFAULT_LIMIT, GridStore, MongoClient, add_ogp, api_root, app, bodyParser, check_archive, compression, content_type, encodings, express, fs, get_from_cache, get_ogpcard, get_zipped, iconv, methodOverride, mongo_url, mongodb, mongodb_credential, mongodb_host, mongodb_port, morgan, re_or_str, redis, repo_backend, request, upload_content, upload_content_data, version, yaml, zlib;
  ...
  add_ogp = function(body, title, author) {
    ...
    return body.replace(/\.\.\/\.\.\//g, 'http://www.aozora.gr.jp/').replace(/\.\.\//g, 'http://www.aozora.gr.jp/cards/').replace(/<head>/, ogp_headers);
  };
    ...
    return my.books.findOne({
      ...
      return request.get(doc[ext + "_url"], {
        ...
        if (err) {
          ...
          return cb(null, add_ogp(iconv.decode(body, encodings[ext]), doc.title, doc.authors[0].full_name));
        }
      });
    });
  };
